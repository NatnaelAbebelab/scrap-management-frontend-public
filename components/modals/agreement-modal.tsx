...
export function AgreementModal({
  ...
  const [ranges, setRanges] = useState<Range[]>([{ min: 0, max: 0, rate: 0, id: "1" }])

  // Add these states
  const [agencyTin, setAgencyTin] = useState("")
  const [agencyFound, setAgencyFound] = useState<boolean | null>(null)
  const [selectedAgencyDetails, setSelectedAgencyDetails] = useState<{ tin: string; businessName: string } | null>(null)

  // Filter agencies based on search
  const filteredAgencies = agencies.filter(
    (agency) => agency.tin.includes(searchTin) || agency.businessName.toLowerCase().includes(searchTin.toLowerCase()),
  )
  ...
  const validateAgency = () => {
    ...
    const foundAgency = agencies.find((agency) => agency.tin === agencyTin)
    if (foundAgency) {
      setAgencyFound(true)
      setSelectedAgencyDetails(foundAgency)
      setSelectedAgency(foundAgency.tin)
    } else {
  ...
  const handleSubmit = () => {
    ...
    if (!agreementName || !selectedAgency || !materialType || !effectiveDate) {
    ...
    const agreementData = {
      agreementName,
      agency: agencies.find((a) => a.tin === selectedAgency),
      materialType,
      effectiveDate,
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Create a new agreement with an agency and define rate ranges.</DialogDescription>
        </DialogHeader>
          ...
          <div className="grid gap-4">
            ...
            <div className="grid grid-cols-1 gap-4">
              ...
              <div className="space-y-2">
                ...
                {agencyFound === false && <p className="text-sm text-red-500">Agency with this TIN not found</p>}
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {proofFile && (
                    <Button variant="ghost" size="icon" onClick={() => setProofFile(null)}>
            ...
            {ranges.map((range, index) => (
              <div key={range.id} className="flex items-center gap-2">
                <div className="grid grid-cols-3 gap-2 flex-1">
                  ...
                  <div>
                    <Label htmlFor={`rate-${range.id}`} className="text-xs">
                      Rate
                    </Label>
                    <Input
                      id={`rate-${range.id}`}
